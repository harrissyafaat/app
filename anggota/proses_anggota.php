<?php
	...
	switch ($pros){
		case "tambah" :
			$kode = 'S0001';
			$q=mysql_query("SELECT besar_simpanan AS pokok FROM t_jenis_simpan WHERE kode_jenis_simpan='$kode'");
			$data = mysql_fetch_array($q);
			$pokok	= $data['pokok'];
			$qu=mysql_query("INSERT INTO t_tabungan VALUES('',$pokok)");		
			$que=mysql_query("SELECT max(kode_tabungan) AS a FROM t_tabungan");
			$dt = mysql_fetch_array($que);

				$qtambah=mysql_query("INSERT INTO t_anggota values('$kode_anggota','$dt[a]','$nama_anggota','$alamat_anggota','$jenis_kelamin','$pekerjaan','$tgl_masuk','$tgl_keluar','$telp','$tempat_lahir','$tgl_lahir','$no_identitas','$photo','$u_entry','$tgl_entri')");
				$qtambah2=mysql_query("INSERT INTO t_simpan VALUES('','$kode','$kode_anggota','$tgl_entri','$pokok','$u_entry','$tgl_entri')");
		...
		case "hapus" :
			$qdelete=mysql_query("DELETE FROM t_anggota WHERE kode_anggota='$kode_anggota'");
			if($qdelete){
				header("location:../index.php?pilih=1.2");
			}else{
